...
from mmdet3d.apis import single_gpu_test, single_gpu_test_stereo
...
def custom_collate_fn(batch, samples_per_gpu=1):
    ...
    for sample in batch:
        if 'results_cam2' in sample and 'results_cam3' in sample:
            flattened_batch.append(sample['results_cam2'][0])
            flattened_batch.append(sample['results_cam3'][0])
        else:
...
def main():
    ...
    data_loader = build_dataloader(
        ...
        dist=distributed,
        shuffle=False)
    data_loader.collate_fn = partial(custom_collate_fn, samples_per_gpu=samples_per_gpu * 2)
    ...
    if not distributed:
        ...
        outputs = single_gpu_test_stereo(model, data_loader, args.show, args.show_dir)
    else:
    ...
    if rank == 0:
        ...
        if args.eval:
            ...
            print(dataset.evaluate_stereo(outputs, **eval_kwargs))
