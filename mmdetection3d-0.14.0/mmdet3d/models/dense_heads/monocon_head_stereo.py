...
from mmdet3d.models.utils.monocon_utils import get_k_local_maximas, save_numpy_array_as_image, resize_numpy_image_by_factor, create_rgb_with_single_channel
...
class MonoConHeadStereo(nn.Module):
    ...
    def loss(self,
        ...
        #                                                      depths,
        #                                                      img_metas)
        # pred_centers_3d_heatmap, target_center_3d_consistency_heatmap = self.get_center_heatmap_consistency(gt_bboxes, gt_labels,
        #                                                      gt_bboxes_3d,
        #                                                      center_heatmap_pred,
        #                                                      center2kpt_offset_pred,
        #                                                      centers2d,
        ...
        # a_kpt_heatmap_pred = to_np(kpt_heatmap_pred)
        ...
        # a_pred_centers_3d_heatmap = to_np(pred_centers_3d_heatmap)
        # a_target_center_3d_consistency_heatmap = to_np(target_center_3d_consistency_heatmap)
        # a_pred_centers_3d_heatmap = to_np(pred_centers_3d_heatmap)
        #
        #
        # fit_center3D_consist_to_orig_image = resize_numpy_image_by_factor(create_rgb_with_single_channel(a_target_center_3d_consistency_heatmap[0] ** 2, 'B'), 4)
        # fit_center3D_to_orig_image = resize_numpy_image_by_factor(create_rgb_with_single_channel(a_pred_centers_3d_heatmap[0] ** 2, 'R'), 4)
        # # fit_to_orig_image = resize_numpy_image_by_factor(np.repeat(a_target_center_3d_consistency_heatmap[0], 3, axis=0), 4)
        # fit_kpts_hm_to_orig_image = resize_numpy_image_by_factor(create_rgb_with_single_channel(a_kpt_heatmap[0] ** 2, 'G'), 4)
        # fit_pred_kpts_hm_to_orig_image = resize_numpy_image_by_factor(
        #     create_rgb_with_single_channel(np.expand_dims(a_kpt_heatmap_pred.sum(1), 1)[0] ** 2, 'B'), 4)
        # # fit_kpts_hm_to_orig_image = resize_numpy_image_by_factor(np.repeat(a_kpt_heatmap[0], 3, axis=0), 4)
        # a_orig_img = [to_np(img_meta['img']) for img_meta in img_metas]
        # image_name = img_metas[0]['filename'].split('/')[-1].split('.')[0]
        # new_img = (normalize_to_uint8(a_orig_img[0]) / 255) / 4 + fit_center3D_consist_to_orig_image + fit_kpts_hm_to_orig_image + fit_center3D_to_orig_image
        # new_img /= new_img.max()
        # new_corners_image = (normalize_to_uint8(a_orig_img[0]) / 255) / 4 + fit_kpts_hm_to_orig_image + fit_pred_kpts_hm_to_orig_image
        #
        # if image_name == '000003':
        #     if not(os.path.isdir(f"/home/matan/Projects/MonoCon/outputs/consistency_outputs/{image_name}")):
        #         os.mkdir(f"/home/matan/Projects/MonoCon/outputs/consistency_outputs/{image_name}")
        #     save_numpy_array_as_image(new_img, f"/home/matan/Projects/MonoCon/outputs/consistency_outputs/{image_name}/R2L_center3D_{str(batch_step).zfill(4)}.png")
        #     save_numpy_array_as_image(new_corners_image,
        #                               f"/home/matan/Projects/MonoCon/outputs/consistency_outputs/{image_name}/corners_pred_{str(batch_step).zfill(4)}.png")
        #     a = 5
        # *****************************************************************************
        # *****************************************************************************
        ...
        # concat_and_show_images(255 * a_pred_centers_3d_heatmap[0][0], 255 * a_pred_centers_3d_heatmap[1][0],
        #                        out_dir="/home/matan/Projects/MonoCon/outputs/debug_loss",
        #                        filename='pred_centers_3d_heatmap_0',
        #                        show=False, suffix="")
        # concat_and_show_images(255 * a_pred_centers_3d_heatmap[2][0], 255 * a_pred_centers_3d_heatmap[3][0],
        #                        out_dir="/home/matan/Projects/MonoCon/outputs/debug_loss",
        #                        filename='pred_centers_3d_heatmap_1',
        #                        show=False, suffix="")
        # concat_and_show_images(255 * a_target_center_3d_consistency_heatmap[0][0], 255 * a_target_center_3d_consistency_heatmap[1][0],
        #                        out_dir="/home/matan/Projects/MonoCon/outputs/debug_loss",
        #                        filename='target_center_3d_consistency_heatmap_0',
        #                        show=False, suffix="")
        # concat_and_show_images(255 * a_target_center_3d_consistency_heatmap[2][0], 255 * a_center_heatmap_pred[3][0],
        #                        out_dir="/home/matan/Projects/MonoCon/outputs/debug_loss",
        #                        filename='target_center_3d_consistency_heatmap_1',
        #                        show=False, suffix="")
        ...
        # # loss_center_heatmap_consistency = self.loss_center_heatmap(pred_centers_3d_heatmap, target_center_3d_consistency_heatmap)
        # loss_corners_heatmap_consistency = self.loss_kpt_heatmap(kpt_heatmap_pred[:,:-1,:,:], corners_consistency_heatmap[:,:-1,:,:])
        # loss_kpts_heatmap_consistency = self.loss_kpt_heatmap(kpt_heatmap_pred, corners_consistency_heatmap)
        # loss_center_heatmap_consistency = self.loss_kpt_heatmap(kpt_heatmap_pred[:,-1:,:,:], corners_consistency_heatmap[:,-1:,:,:])
        #
        #
        # if self.dim_aware_in_loss:
        ...
        # loss_corners_heatmap_consistency *= c / 4
    ...
    def get_center_heatmap(self, gt_bboxes, gt_labels,
        ...
        *batch_dets, ys, xs = get_topk_from_heatmap(
        ...
        # scores, ys, xs = get_k_local_maximas(center_heatmap_pred)
        pred_centers = torch.cat([xs.unsqueeze(-1).float(), ys.unsqueeze(-1).float()], dim=-1)
        center2kpt_offset_pred = transpose_and_gather_feat(center2kpt_offset_pred, batch_index)
        pred_centers_3d = pred_centers + center2kpt_offset_pred[:,:,-2:]
        for batch_id in range(bs):
            # dest_id the is stereo match id: 0 -> 1, 1 -> 0 ...
            ...
            distances = torch.cdist(gt_centers, pred_centers_3d[batch_id])
            closest_indices = torch.argmin(distances, dim=1)
            closest_pred_centers_3d = pred_centers_3d[batch_id][closest_indices]
            for j, ct in enumerate(closest_pred_centers_3d):
    ...
    def get_center_heatmap_consistency(self, gt_bboxes, gt_labels,
                           gt_bboxes_3d,
                           center_heatmap_pred,
                           center2kpt_offset_pred,
                           centers2d,
        ...
        calibs = [img_meta['cam_intrinsic'] for img_meta in img_metas]
        # objects as 2D center points
        target_center_3d_consistency_heatmap = gt_bboxes[-1].new_zeros([bs, self.num_classes, feat_h, feat_w])
        pred_center_3d_heatmap = gt_bboxes[-1].new_zeros([bs, self.num_classes, feat_h, feat_w])
        ...
        *batch_dets, ys, xs = get_topk_from_heatmap(
        ...
        pred_centers = torch.cat([xs.unsqueeze(-1).float(), ys.unsqueeze(-1).float()], dim=-1)
        center2kpt_offset_pred = transpose_and_gather_feat(center2kpt_offset_pred, batch_index)
        pred_centers_3d = pred_centers
        pred_centers_3d = pred_centers.clone()
        for batch_id in range(bs):
            # dest_id the is stereo match id: 0 -> 1, 1 -> 0 ...
            dest_id = batch_id + (-1) ** batch_id
            gt_bbox = gt_bboxes[batch_id]
            ...
            closest_pred_centers_3d = pred_centers_3d[batch_id][closest_indices]
            for j, ct in enumerate(closest_pred_centers_3d):
                # get x of the corresponded image
                delta_x_pixels = get_delta_x_pixels(P_source=calibs[batch_id], P_dest=calibs[dest_id], depth=depths[batch_id][j])
                ctx_int, cty_int = ct.int()
                dest_ctx_int = ctx_int + torch.round(delta_x_pixels * width_ratio).int()
                scale_box_h = (gt_bbox[j][3] - gt_bbox[j][1]) * height_ratio
                scale_box_w = (gt_bbox[j][2] - gt_bbox[j][0]) * width_ratio
                radius = gaussian_radius([scale_box_h, scale_box_w],
                                         min_overlap=0.3)
                radius = max(0, int(radius))
                ind = gt_label[j]
                # e.g. if batch_id = 0 (left image) -> genereate point for image 1 (right image) after projection matrix transformation (only x value is changing)
                gen_gaussian_target(target_center_3d_consistency_heatmap[dest_id, ind],
                                    [dest_ctx_int, cty_int], radius)
                # generate the projected 3d map
                gen_gaussian_target(pred_center_3d_heatmap[batch_id, ind], [ctx_int, cty_int], radius
                                    )
        return pred_center_3d_heatmap, target_center_3d_consistency_heatmap
    ...
    def get_corners_heatmap(self, gt_bboxes, gt_labels,
        ...
        kpts_local_maximas = [get_k_local_maximas(kpt_heatmap_pred[:, d, :, :].unsqueeze(1)) for d in range(self.num_kpt)]
        for k in range(self.num_kpt):
            ...
            #     continue
            batch_ids, ys, xs = kpts_local_maximas[k]
    ...
    def get_stereo_indices(self, gt_bboxes, gt_labels,
        ...
        # objects as 2D center points
        center_heatmap_target = gt_bboxes[-1].new_zeros([bs, self.num_classes, feat_h, feat_w])
        _, ys, xs = get_k_local_maximas(center_heatmap_pred)
        pred_centers = torch.cat([xs.unsqueeze(-1).float(), ys.unsqueeze(-1).float()], dim=-1)
        for batch_id in range(bs):
            dest_id = batch_id + (-1) ** batch_id
            gt_bbox = gt_bboxes[batch_id]
    ...
    def get_targets(self, gt_bboxes, gt_labels,
        ...
        kpt_heatmap_target = gt_bboxes[-1].new_zeros([bs, 1, feat_h, feat_w])
        ...
        for batch_id in range(bs):
            ...
            for j, ct in enumerate(gt_centers):
                ...
                for k in range(self.num_kpt):
                    ...
                    gen_gaussian_target(kpt_heatmap_target[batch_id, -1],
    ...
    def decode_heatmap_to_3d_pts(self,
        ...
        *batch_dets, ys, xs = get_topk_from_heatmap(
        ...
        center2kpt_offset = transpose_and_gather_feat(center2kpt_offset_pred, batch_index)
        # get the last coordinates (x,y of the 9th which are the offset of the 3d center
        center2kpt_offset = center2kpt_offset.view(batch, k, self.num_kpt * 2)[..., -2:]
        # get the projected (feature map coordinates) 3d center in feature map from the predicted 2d center with the offset
        center2kpt_offset[..., ::2] += xs.view(batch, k, 1).expand(batch, k, 1)
        center2kpt_offset[..., 1::2] += ys.view(batch, k, 1).expand(batch, k, 1)
        kpts = center2kpt_offset
        # get the
        ...
        # 1.5 get projected center
        center3d_projected = kpts  # (b, k, 2)

        # 2.5 recover box3d_center from center2d and depth
        P = center3d_projected.new_tensor(camera_intrinsic)
        center3d = (depth * (center3d_projected - torch.transpose(P[:, :2, 2:3], 1, 2)) - torch.transpose(P[:, :2, 3:], 1,                                                                                     2)) / P[:, :1, :1]        # 3. compose 3D box
        center3d = torch.cat([center3d, depth], dim=-1)
        bs, _, feat_h, feat_w = center_heatmap_pred.size()
        ...
        for batch_id in range(batch):
            if len(gt_bboxes_3d[batch_id]) == 0:
                ...
                continue
            dest_id = batch_id + (-1) ** batch_id
            gt_center_3d = gt_bboxes_3d[batch_id].center.to(center3d_projected.device)
        ...
        # print(closest_pred_centers_batch)
    ...
    def forward_train(self,
        ...
        losses = self.loss(*loss_inputs, gt_bboxes_ignore=gt_bboxes_ignore, batch_step=kwargs['batch_step'])
