...
class MonoConHead(nn.Module):
    def __init__(self,
        ...
        self.kptNet_heatmap_head = self._build_head(in_channel, feat_channel, self.num_kptNet)
        self.kptNet_depth_head = self._build_head(in_channel, feat_channel, self.num_kptNet)
        ...
        self.loss_keypointnet = build_loss(loss_keypointnet)
    ...
    def forward_single(self, feat):
        ...
        kptNet_heatmap_pred = self.kptNet_heatmap_head(feat)
        kptNet_depth_pred = self.kptNet_depth_head(feat)
        ...
        return center_heatmap_pred, wh_pred, offset_pred, center2kpt_offset_pred, kpt_heatmap_pred, \
               ...
               kptNet_heatmap_pred, kptNet_depth_pred
    ...
    @force_fp32(apply_to=('center_heatmap_preds', 'wh_preds', 'offset_preds', 'center2kpt_offset_preds',
                          ...
                          'alpha_offset_preds', 'depth_preds', 'kptNet_heatmap_pred', 'kptNet_depth_pred'))
    def loss(self,
        ...
        assert len(center_heatmap_preds) == len(wh_preds) == len(offset_preds) \
               ...
               == len(dim_preds) == len(alpha_cls_preds) == len(alpha_offset_preds) == len(kptNet_heatmap_pred) \
               == len(kptNet_depth_pred) == 1
        ...
        kptNet_heatmap_pred = kptNet_heatmap_pred[0]
        kptNet_depth_pred = kptNet_depth_pred[0]
        ...
        loss_keypointNet = self.loss_keypointnet(kptNet_heatmap_pred, kptNet_depth_pred)
        ...
        return dict(
            ...
            loss_keypointNet=loss_keypointNet
        )
