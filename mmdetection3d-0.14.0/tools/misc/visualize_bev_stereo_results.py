...
def main(new_coco_json=None):
    ...
    if getattr(dataset, 'show_bev', None) is not None:
        ...
        if eval_pipeline:
            dataset.show_bev_stereo(results, out_dir=args.output_dir, show=args.show, pipeline=eval_pipeline)
        else:
            dataset.show_bev_stereo(results, out_dir=args.output_dir, show=args.show)  # use default pipeline
    else:
