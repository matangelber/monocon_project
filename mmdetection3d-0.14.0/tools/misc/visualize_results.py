...
def main(new_coco_json=None):
    ...
    if getattr(dataset, 'show', None) is not None:
        ...
        if eval_pipeline:
            dataset.show(results, output_dir=args.output_dir, show=args.show, pipeline=eval_pipeline)
        else:
            dataset.show(results, output_dir=args.output_dir, show=args.show)  # use default pipeline
    else:
